...
# importing icon from internet
try:
    import requests

    urllib.request.urlretrieve(
        "https://res.cloudinary.com/dewbkmgth/image/upload/v1607090275/ai_mst/app_logo_kgvkv9.ico",
...
class Install_App(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        install_menu_background = "#0D19A3"
        highlighted_menu_background = "#3b43a3"
        install_menu_text = "#00ff7f"
        ...
        def allowed_installation():
                ...
                # pass-manager-png
                urllib.request.urlretrieve("https://res.cloudinary.com/dewbkmgth/image/upload/v1607446987/ai_mst/pass_manager_lock_r3kjad.png",
                                           "KI - SAFE/lock_img_cached.jpg")

                urllib.request.urlretrieve("https://res.cloudinary.com/dewbkmgth/image/upload/v1607090275/ai_mst/app_logo_kgvkv9.ico",
                                           "KI - SAFE/app_logo.ico")

                # pass-gen-png
                urllib.request.urlretrieve(
                    "https://res.cloudinary.com/dewbkmgth/image/upload/v1607449144/ai_mst/pass_gen_ozdzwc.png",
                    "KI - SAFE/pass_gen.png")

                # notes-man-png
                urllib.request.urlretrieve(
                    "https://res.cloudinary.com/dewbkmgth/image/upload/v1607456662/ai_mst/notes_man_bmkyt8.png",
                ...
                urllib.request.urlretrieve(
                    "https://res.cloudinary.com/dewbkmgth/image/upload/v1607445882/ai_mst/youtube_logo_b2dhve.png",
                    "KI - SAFE/youtube_logo.png")

                urllib.request.urlretrieve(
                    "https://res.cloudinary.com/dewbkmgth/image/upload/v1607446431/ai_mst/twitter_logo_u3rpbe.png",
                    "KI - SAFE/twitter_logo.png")

                urllib.request.urlretrieve(
                    "https://res.cloudinary.com/dewbkmgth/image/upload/v1607446431/ai_mst/github_logo_r8cglf.png",
        ...
        app_name_head = ttk.Label(app_header_frame, text="KI - SAFE", font=terms_cond_font, foreground=install_menu_text,
        ...
        allow_btn = ttk.Button(user_choice_frame, text="I Agree, Continue >", command=call_the_app, style="user_choice.TButton")
...
class HomePage(tk.Frame, tk.Tk):
    def __init__(self, container, controller, **kwargs):
        ...
        f_1 = tk.Frame(copy_frame, background="Red")
        f_2 = tk.Frame(copy_frame, background="blue")
        f_3 = tk.Frame(copy_frame, background="yellow")

        app_tagline = ttk.Label(copy_frame, text="Your data is \U0001F512 with us", background=background_color, foreground=frame_colors, font=('Helvetica', 30, 'bold'))
        app_tagline.place(x=500, y=10)

        # packing the frames onto the screen
        f_1.pack(side="left", padx=(150, 0), pady=(100, 50))
        f_2.pack(side="left", padx=(50, 50), pady=(100, 50))
        f_3.pack(side="left", padx=(0, 150), pady=(100, 50))

        # labels for the three different frames
        label_1 = tk.Label(f_1, text="Password Manager", background=frame_colors, height=5, width=18, foreground=text_color, font=("Helvitca", 20, "bold"))
        label_2 = tk.Label(f_2, text="Password Generator", background=frame_colors, height=5, width=18, foreground=text_color, font=("Helvitca", 20, "bold"))
        label_3 = tk.Label(f_3, text="Notes Manager", background=frame_colors, height=5, width=18, foreground=text_color, font=("Helvitca", 20, "bold"))
        ...
        button_sec_2 = ttk.Button(f_2, text="Continue \U000027F6", command=lambda: controller.show_frame(PassGenerator), style="arrow_btn.TButton")
        button_sec_2.pack(side="left", expand=True, fill='x')

        button_sec_3 = ttk.Button(f_3, text="Continue \U000027F6", command=lambda: messagebox.showwarning("ERROR 404 :(", "Work in progress"))
        button_sec_3.pack(side="left", expand=True, fill='x')

        lower_bar = tk.Frame(self, background=frame_colors)
        lower_bar.pack(side="top", expand=True, fill="x")

        # status_label = ttk.Label(lower_bar, text="I AM THE FUCKING KING...")
        # status_label.pack()

        tag_line = ttk.Label(lower_bar, text="We're here for you", font=("Helvetica", 20, "bold", "roman"), background=frame_colors, foreground=background_color)
        tag_line.grid(row=0, column=0, padx=(50, 50))
        comment_label = ttk.Label(lower_bar, text="Follow us at: //", font=("Helvetica", 20, "bold", "italic"), background=frame_colors, foreground=background_color)
        ...
        twitter_label.bind("<Button-1>", lambda e: callback("http://www.twitter.com"))
        ...
        github_label.bind("<Button-1>", lambda e: callback("http://www.github.com"))
...
class PassManager(ttk.Frame, tk.Tk):
    def __init__(self, container, controller, **kwargs):
        ...
        panel.image = logo
        panel.grid(row=0, column=0, padx=(50, 50), pady=(50, 50))
        ...
        def save_data():
            ...
            if "facebook" in website.lower():
                ...
                fb_frame = ttk.Frame(store_credentials_frame, background=background_color)
                ...
                fb_label = ttk.Label(fb_frame, text=f"User name: {u_name}\nPassword: {u_pass}", background=background_color, foreground=text_colors)
        ...
        url_prompt = ttk.Label(user_data_frame, text="Website:", justify="left", background=background_color, foreground=text_colors)
        user_name_prompt = ttk.Label(user_data_frame, text="Name:", justify="left", background=background_color, foreground=text_colors)
        user_pass_prompt = ttk.Label(user_data_frame, text="Password:", justify="right", background=background_color, foreground=text_colors)
    ...
    def __init__(self, container, controller, **kwargs):
        ...
        def show_credentials():
            if pass_gen_cnt == 1:
                ...
                diff_pass_gen_btn.grid(row=2, column=2, padx=(50, 0))
            else:
                pass
            ...
            global pass_gen_id
            pass_gen_id = ttk.Label(pass_generator_frame, text=self.email, background=background_color, foreground=frame_colors)
            pass_gen_pass = ttk.Label(pass_generator_frame, text=self.password, background=background_color, foreground=frame_colors)
        ...
        home_button.grid(row=1, column=0, padx=(50, 50), pady=(50, 50))
        ...
        pass_manager_button.grid(row=2, column=0, padx=(50, 50), pady=(20, 20))

        pass_generator_button = ttk.Button(side_nav_frame, text="Password Generator",
                                           command=lambda: controller.show_frame(PassGenerator)
                                           )
        pass_generator_button.grid(row=3, column=0, padx=(50, 50), pady=(20, 20))

        notes_manager_button = ttk.Button(side_nav_frame, text="Notes Manager",
                                          command=lambda: controller.show_frame(NotesManager)
                                          )
        notes_manager_button.grid(row=4, column=0, padx=(50, 50), pady=(20, 20))
        ...
        user_name_pass_gen = ttk.Label(pass_generator_frame, text="Name:", justify="left", background=background_color, foreground=text_color)
        pass_gen_u_id_prompt = ttk.Label(pass_generator_frame, text="User ID:", justify="left", background=background_color, foreground=text_color)
        pass_gen_u_pass_prompt = ttk.Label(pass_generator_frame, text="Password:", justify="right", background=background_color, foreground=text_color)
        ...
        pass_gen_btn.grid(row=1, column=2, padx=(50, 0), pady=(30, 0))
        ...
        scale = ttk.Scale(pass_generator_frame, orient="horizontal", from_=0, to=10, command=handle_scale_change,
                          variable=current_value)
        scale.grid(row=5, column=0, sticky="EW", columnspan=4, padx=(150, 50), pady=(25, 25))
        ...
        pass_gen_tip_1 = ttk.Label(tips_pass_gen_sec, text="1. Longer passwords are better: 8 characters is a starting point", background=slight_dark_bg, foreground=text_color)
        pass_gen_tip_1.grid(row=0, column=0, padx=(10, 10), pady=(50, 50), sticky="W")

        pass_gen_tip_2 = ttk.Label(tips_pass_gen_sec, text="2. Don't recycle your passwords", background=slight_dark_bg, foreground=text_color)
        pass_gen_tip_2.grid(row=1, column=0, padx=(10, 10), pady=(0, 50), sticky="W")

        pass_gen_tip_3 = ttk.Label(tips_pass_gen_sec, text="3. Avoid common words and character combinations in your password", background=slight_dark_bg, foreground=text_color)
        pass_gen_tip_3.grid(row=2, column=0, padx=(10, 10), pady=(0, 50), sticky="W")

        pass_gen_tip_4 = ttk.Label(tips_pass_gen_sec, text="4. Use a password manager to keep track of your passwords", background=slight_dark_bg, foreground=text_color)
        pass_gen_tip_4.grid(row=3, column=0, padx=(10, 10), pady=(0, 75), sticky="W")


class NotesManager(ttk.Frame):
    def __init__(self, container, controller, **kwargs):
        super().__init__(container, **kwargs)
        self.l_files = []
        ...
        def new_file_func():
            ...
            new_win_controls = ttk.Frame(new_window)
            ...
            file_name_label = ttk.Label(new_win_controls, text="Filename:")
            file_title_label = ttk.Label(new_win_controls, text="Title:")
            self.file_name = ttk.Entry(new_win_controls, textvariable=f_name, width=10)
            file_title = ttk.Entry(new_win_controls, textvariable=f_title, width=10)

            file_name_label.grid(row=0, column=0, pady=(20, 20), sticky="W")
            file_title_label.grid(row=1, column=0, pady=(20, 20), sticky="W")
            ...
            def retrieve_input():
                ...
                print(f_name)
                with open(self.file_name.get() + '.txt', 'w') as writer:
            ...
            file_save_btn.grid(row=2, column=0, pady=(50, 50))
        ...
        # a side-nav frame
        side_nav_frame = tk.Frame(self, background="blue")
        ...
        # creating a label to display image
        panel = ttk.Label(side_nav_frame, image=logo)
        ...
        # social media icons section at the bottom
        social_media_icons_frame = tk.Frame(side_nav_frame, background="cyan")
        social_media_icons_frame.grid(row=5, column=0, padx=(20, 20), pady=(50, 10))

        facebook_icon = ttk.Label(social_media_icons_frame, text="SOME ICONS HERE!!!")
        facebook_icon.pack()

        banner_frame = ttk.Frame(self)
        banner_frame.grid(row=0, column=1, sticky="N", padx=(150, 50), pady=(50, 50))

        slogan = ttk.Label(banner_frame, text="YOUR SAVED NOTES WILL SHOW HERE")
        slogan.pack(side="top")

        # saved files
        note_manager_frame = ttk.Frame(self)
        note_manager_frame.grid(row=0, column=1)
        cnt = 0
        print(self.l_files)
        ...
        # new file button
        new_file_btn = ttk.Button(note_manager_frame, text="+", command=new_file_func)
        new_file_btn.grid(row=10, column=1)


#
# app_installation = Install_App()
# app_installation.mainloop()
if not os.path.exists('./KI - SAFE'):
    app = Install_App()
    app.mainloop()
else:
